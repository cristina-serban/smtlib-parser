/**
 * \file ast_symbol_util.h
 * \brief Utility classes for the symbol stack.
 * \author Cristina Serban <cristina.serban89@gmail.com>
 */

#ifndef SMTLIB_PARSER_SYMBOL_UTIL_H
#define SMTLIB_PARSER_SYMBOL_UTIL_H
...
namespace smtlib {
    ...
    public:
        ...
        SortInfo(std::string name, unsigned long arity,
                 sptr_t<ast::AstNode> source) : arity(arity) {
            this->name = name;
            this->source = source;
        }
    ...
    public:
        ...
        FunInfo(std::string name,
                sptr_v<ast::Sort>& signature,
                sptr_t<ast::Term> body,
                sptr_t<ast::AstNode> source) : body(body) {
            init(name, signature, source);
        }
    ...
    public:
        ...
        VarInfo(std::string name,
                sptr_t<ast::Sort> sort,
                sptr_t<ast::AstNode> source) : sort(sort) {
            this->name = name;
            this->source = source;
        }
        ...
        VarInfo(std::string name,
                sptr_t<ast::Sort> sort,
                sptr_t<ast::Term> term,
                sptr_t<ast::AstNode> source) : sort(sort), term(term) {
            this->name = name;
            this->source = source;
        }
    };
}
#endif //SMTLIB_PARSER_SYMBOL_UTIL_H