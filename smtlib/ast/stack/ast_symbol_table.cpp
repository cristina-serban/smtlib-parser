...
unordered_map<string, sptr_t<SortInfo>>& SymbolTable::getSorts() {
    return sorts;
}

unordered_map<string, sptr_v<FunInfo>>& SymbolTable::getFuns() {
    return funs;
}

unordered_map<string, sptr_t<VarInfo>>& SymbolTable::getVars() {
    return vars;
}
